...
# File Name: PG&E E-TOU-B Tariff Vectors Creator.R
# File Location: setwd("~/Desktop/Suncode 2019/Rates")
# Project: Suncode 2019
# Description: Converts PG&E E-TOU-B rate into time-series vectors for use in Switch's cost and constraint matrices.
...
PGE_ETOUB_Cost_Vectors.df <- data.frame(Date_Time = seq.POSIXt(Start_Date_Time, End_Date_Time, by = paste(Data_Timestep_Length, "min")))


#### Create Vectors for Energy and Demand Charges ####

# Values are for PG&E E-TOU-B, in US $.

Summer_Peak_Rate = 0.36452
Summer_Off_Peak_Rate = 0.26146

Winter_Peak_Rate = 0.22705
Winter_Off_Peak_Rate = 0.20825
...
PGE_ETOUB_Cost_Vectors.df <- PGE_ETOUB_Cost_Vectors.df %>%
...
PGE_ETOUB_Cost_Vectors.df <- PGE_ETOUB_Cost_Vectors.df %>%
 mutate(Winter_Off_Peak_Binary = ifelse(Season == "Winter" &
                      Winter_Peak_Binary == 0, 1, 0))


# PG&E Daylight Savings Time Adjustment - Shift Time Period Definitions back 1 Hour in March/April and October/November

# In PG&E Schedule E-TOU-C (NEW) , the peak/partial-peak/off-period time definitions do not shift in sync with Daylight Savings Time.
...
PGE_ETOUB_Cost_Vector_Check.df <- PGE_ETOUB_Cost_Vectors.df %>%
 mutate(Check_Sum = Summer_Peak_Binary + Summer_Off_Peak_Binary + 
      Winter_Peak_Binary + Winter_Off_Peak_Binary) %>%
 filter(Check_Sum != 1)
# This dataframe should be empty (0 observations).

rm(PGE_ETOUB_Cost_Vector_Check.df)
...
# All data
write.csv(PGE_ETOUB_Cost_Vectors.df, file.path(Clean_Rate_Data_WD, paste0(Data_Timestep_Length, "-Minute Data"), "Dataframe Format", paste0(Data_Year, "_PGE_ETOUB_Cost_Dataframe.csv")), row.names = FALSE)

# Month Value Column Only
write.table(PGE_ETOUB_Cost_Vectors.df$Month, file.path(Clean_Rate_Data_WD, paste0(Data_Timestep_Length, "-Minute Data"), "Vector Format", paste0(Data_Year, "_PGE_ETOUB_Month_Vector.csv")), 
      row.names = FALSE, col.names = FALSE, sep = ',')

# Energy Rates Variable Column
write.table(PGE_ETOUB_Cost_Vectors.df$Energy_Rates, file.path(Clean_Rate_Data_WD, paste0(Data_Timestep_Length, "-Minute Data"), "Vector Format", paste0(Data_Year, "_PGE_ETOUB_Energy_Rates_Vector.csv")), 
      row.names = FALSE, col.names = FALSE, sep = ',')